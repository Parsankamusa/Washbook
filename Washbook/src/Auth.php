...
class Auth {
    ...
    public static function user() {
        ...
        if (!empty($user)) {
            ...
            $user->branch = Database::table("branches")->where("id", $user->branchid)->first();
        }
    ...
    public static function login($username, $password, $options = array()) {
        ...
        $user = Database::table(config('auth.table'))->where("email",$username)->where("role","Admin")->orWhere("email",$username)->where("role","Owner")->orWhere("email",$username)->where("role","Manager")->first();
            ...
            if(hash_compare($user->$passwordColumn, self::password($password))){
                ...
                self::authenticate($user);
                 return response()->json(array(
                    "status" => "success",
                    "notify" => false,
                    "callback" => "redirect('".url('Overview@get')."', true)")
                );
