...
if(! function_exists('icon')) {
     ...
     */
    function icon($name) {
        $icon = config("app.icons").$name.".svg";
        if (file_exists($icon)) {
...
if(! function_exists('carmake')) {
    ...
    function carmake($makeid) {
        if (is_int((int) $makeid) && !empty($makeid)) {
            $make = Database::table('makes')->where('makeid', $makeid)->first();
            if (!empty($make)) {
                return $make->name;
            }else{
                return $makeid;
...
if(! function_exists('carmodel')) {
    ...
    function carmodel($modelid) {
        if (is_int((int) $modelid) && !empty($modelid)) {
            $model = Database::table('models')->where('modelid', $modelid)->first();
            if (!empty($model)) {
                return $model->name;
            }else{
                return $modelid;
